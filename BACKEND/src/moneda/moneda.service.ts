...
export class MonedaService {
    constructor(@InjectRepository(Moneda) private monedaRepository : Repository<Moneda>){}


    async Validar_Moneda(codigo : string):Promise<boolean>{
        const moneda = await this.monedaRepository.findOne({where:{codigo:ILike(`%${codigo}%`)}})
        if(moneda){
            return true
        }
    ...
    async Crear_Moneda(CreatemonedaDto: CreateMonedaDto): Promise<Moneda>{
        ...
        if(moneda){
            throw new NotFoundException(`Este Codigo Tributario ya esta Creado ${CreatemonedaDto.codigo}`)
        }
    ...
    async Obtener_Moneda(codigo: string): Promise<Moneda>{
        ...
        throw new NotFoundException(`Este Codigo Tributario No esta Creado ${codigo}`)
    }

    async Eliminar_Moneda(codigo: string): Promise<Moneda>{
        return await this.monedaRepository.remove(await this.Obtener_Moneda(codigo))
    }

    async Actualizar_Moneda(codigo: string,updatemoneda : UpdateMonedaDto): Promise<Moneda>{
