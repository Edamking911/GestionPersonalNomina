...
export class DepartamentosService {
  constructor(
    ...
    private departamento: Repository<Departamento>,
  ) {}
  ...
  async ValidarDepartamento(nombre: string): Promise<boolean> {
    const departamento = await this.departamento.findOne({
      where: { nombre: nombre },
    });
    if (departamento) {
      return true;
  ...
  async Buscar_Departamento(nombre: string): Promise<Departamento> {
    const existe = await this.departamento.findOne({
    ...
    if (existe) {
      ...
      throw new NotFoundException(
        `Este Departamento no ha sido registrado ${nombre}`,
      );
    }
  }
  ...
  async Eliminar_Departamento(nombre: string): Promise<Departamento> {
    return await this.departamento.remove(
      await this.Buscar_Departamento(nombre),
    );
  }
