...
export class TasaCambioService {
    constructor(@InjectRepository(TasaCambio) private TasacambioRepository: Repository<TasaCambio>,
    ...
    async Validar_Moneda(codigo : string): Promise<Moneda>{
        ...
        throw new NotFoundException(`Este Codigo Tributario no se encuentra creado ${codigo}`)
    }
    ...
    async Obtener_Tasa(tasa: number): Promise<TasaCambio>{
        const Tasa = await this.TasacambioRepository.findOne({where:{tasa:tasa}})
        if(Tasa){
            return Tasa
        }
    ...
    async Actualizar_Tasa(tasa:number, updateDTOTasa:UpdateTasaCambioDto):Promise<TasaCambio>{
