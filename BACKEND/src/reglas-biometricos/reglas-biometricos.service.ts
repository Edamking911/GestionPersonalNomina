...
export class ReglasBiometricosService {
  ...
  private formatearHoras(horas: number): string {
   ...
   async evaluarEmpleado(
    ...
    // =====================================================
    // ✅ CÁLCULO CORREGIDO DE HORAS DIURNAS/NOCTURNAS Y EXTRAS
    // =====================================================
    ...
    if (salidaHorarioMin <= HORA_NOCTURNA) {
      ...
      // Turno con componente nocturno (ej. 8:00 AM - 8:00 PM)
      ...
      horasNocturnas = Math.max(0, (Math.min(tiempoTrabajadoMin, totalNormalMin) - parteDiurnaNormal) / 60);
    ...
    const horasExtra = Math.round(Math.max(0, (tiempoTrabajadoMin - duracionTurnoMin) / 60) * 100) / 100;
